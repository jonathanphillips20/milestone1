...
public class Client {
	...
	public static void main(String args[]) {
		if (args.length < 3) {
			System.out.println("Incorrect use. use java Client <server> <port> <timeout> <inputFile(optional)>");
			return;
		}
			...
			try {
				...
				while ((cLine = br.readLine()) != null) {
					if (!cLine.startsWith("%") && !cLine.trim().equals("")) {
						...
						try {
							if (s[0].equals("0")) {
								client.main(Integer.parseInt(s[1]), s[2].toCharArray(), s[3].toCharArray(), Short.parseShort(s[4]));
							} else if (s[0].equals("1")) {
								client.register(Integer.parseInt(s[1]), s[2].toCharArray(), s[3].toCharArray());
							...
							System.out.println("Invalid input format on line: \"" + cLine + "\"");
							return;
						}
					}
				}
	...
	private void main() {
		...
		try {
			...
			System.out.println("Invalid ID. ID must be comprised of only numbers\nand must be in the range 0 - 2^31-1(inclusive)");
			kbreader.close();
			return;
		}
		System.out.print("\tEnter Password: ");
		password = kbreader.next().toCharArray();
		System.out.print("\tEnter name: ");
		name = kbreader.next().toCharArray();
		this.register(id, password, name);
		...
		System.out.print("\tEnter Password: ");
		password = kbreader.next().toCharArray();
		System.out.print("\tEnter vote (number): ");
		try {
			vote = kbreader.nextShort();
		} catch (InputMismatchException e) {
			System.out.println("Invalid voteNum. Must be comprised of only numbers and must be in the range 1 - 2^15-1(inclusive)");
			kbreader.close();
			return;
		}
	...
	private void main(int id, char[] password, char[] name, short vote) {
		this.register(id, password, name);
		this.vote(vote, id, password);
	}
	...
	public byte register(int id, char[] password, char[] name) {
		...
		if (password.length > 16 || name.length > 16 ) { // char[] restricted to 16 long
			System.out.println("Invalid input. Password, Name and District limited 16 chars");
			return (byte) -2;
		}
		byte[] entryArray = (new Entry(id, password, name)).toByteArray();
