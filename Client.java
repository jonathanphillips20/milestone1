...
public class Client {
	...
	private void main() {
		...
		System.out.print("\tEnter id: ");		          try{id = kbreader.nextInt();}catch(InputMismatchException e){System.out.println("Invalid ID. ID must be comprised of only numbers\nand must be in the range 0 - 2^31-1(inclusive)"); kbreader.close(); return;}
		System.out.print("\tEnter Password: ");		    password = kbreader.next().toCharArray();
		System.out.print("\tEnter vote (number): ");	try{vote = kbreader.nextShort();}catch(InputMismatchException e){System.out.println("Invalid voteNum. Must be comprised of only numbers and must be in the range 1 - 2^15-1(inclusive)"); kbreader.close(); return;}
